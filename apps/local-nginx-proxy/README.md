...
Publishes **https://local-nginx.dev8.sedware.net** on the public Envoy edge and
...
```
Internet ─▶ public Envoy edge (TLS, *.dev8.sedware.net wildcard, host local-nginx.dev8)
         ─▶ HTTPRoute URLRewrite Host: local-nginx.local.dev8.sedware.net
         ─▶ Backend local-edge (fqdn dev-manager.nb.dev8.sedware.net:443) + origin TLS
         ─▶ BackendTLSPolicy re-encrypt (verify *.local.dev8 wildcard, System trust)
...
service a real internet HTTPS URL under `dev8.sedware.net`, the request must enter
...
- The public `public-dev` Gateway already serves `*.dev8.sedware.net` with the
...
- The `HTTPRoute` rewrites the request `Host` to
  `local-nginx.local.dev8.sedware.net` and forwards to an Envoy Gateway `Backend`
  CRD (`local-edge`) whose endpoint is the stable NetBird peer FQDN
  `dev-manager.nb.dev8.sedware.net:443` — the Local Private Edge. CoreDNS resolves
  that FQDN to the current overlay IPv4 at runtime (`*.nb.dev8` forward), so no
...
- A `BackendTLSPolicy` re-encrypts to the local edge and verifies its Let's
  Encrypt `*.local.dev8.sedware.net` wildcard. The rewritten hostname is both the
...
- **Upstream peer FQDN** (`Backend` → `dev-manager.nb.dev8.sedware.net:443`). This
