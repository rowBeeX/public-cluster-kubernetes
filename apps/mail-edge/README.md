...
- **Inbound MX** — `POSTFIX_relay_domains=dev10.sedware.net` accepts mail for the
  domain; `POSTFIX_transport_maps=inline:{ dev10.sedware.net=smtp:[<stalwart>]:25 }`
...
- **TLS** — STARTTLS on :25 using the cert-manager `Certificate` `mail-edge-tls`
  (`mail.dev10.sedware.net`, DNS-01 via ClusterIssuer `letsencrypt-dev`). The
...
- **Local Stalwart backend.** `POSTFIX_transport_maps` forwards `dev10.sedware.net`
  to `smtp:[dev-manager.nb.dev10.sedware.net]:25` — the stable NetBird peer FQDN of
...
- **DNS.** An MX record `dev10.sedware.net` → `mail.dev10.sedware.net` and an A/AAAA
  record `mail.dev10.sedware.net` → the public gateway IP(s), plus SPF/DKIM/DMARC,
