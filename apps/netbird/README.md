...
| netbird-relay Deployment | WebSocket-Relay (§8/§21), Port 33080, öffentlich `rels://netbird-relay.dev8.sedware.net:443` |
...
2. In der Management-Config (`netbird-config` → `config.yaml`) den `Relay`-Block
   ...
   ```json
   "Relay": {
       "Addresses": ["rels://netbird-relay.dev8.sedware.net:443"],
       "CredentialsTTL": "24h",
       "Secret": "<gleicher Wert wie NB_AUTH_SECRET>"
   }
   ```

3. DNS-Record `netbird-relay.dev8.sedware.net` auf die Gateway-Nodes zeigen lassen
   (analog zu den übrigen `*.dev8.sedware.net` Hosts).

## Zugang

- Dashboard: `https://netbird.dev8.sedware.net/` — öffentlich über den Envoy-Edge
