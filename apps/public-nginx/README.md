...
A minimal static nginx served at **https://public-nginx.dev13.sedware.net** over
...
```
Internet ─▶ public Envoy edge (TLS, *.dev13.sedware.net wildcard cert) ─▶ public-nginx :8080
```

## Why it "just works"

The public `public-dev` Gateway already has an `https` listener for
`*.dev13.sedware.net` with the `public-dev-wildcard-tls` LE certificate, and the
