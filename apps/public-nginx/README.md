...
A minimal static nginx served at **https://public-nginx.dev12.sedware.net** over
...
```
Internet ─▶ public Envoy edge (TLS, *.dev12.sedware.net wildcard cert) ─▶ public-nginx :8080
```

## Why it "just works"

The public `public-dev` Gateway already has an `https` listener for
`*.dev12.sedware.net` with the `public-dev-wildcard-tls` LE certificate, and the
