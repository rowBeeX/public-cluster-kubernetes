...
| `EXC-postfix-writable-rootfs` | platform-mail | `readOnlyRootFilesystem:true` ist für `mail-edge/postfix` nicht erreichbar (#11/#12): das `boky/postfix`-Image erzeugt `/etc/postfix` bei jedem Start neu, und Postfix schreibt `/var/lib/postfix/prng_exch`. Mit derselben Begründung in der Conftest-`_rootfs_exceptions`-Allowlist geführt. | Beschreibbarer Image-Layer im Container; der Queue-Zustand liegt auf der PVC `mail-edge-spool`, TLS ist ein read-only Mount. | 2026-10-01 |
