...
| `EXC-adguard-privileged-psa` | platform-net | `app-adguard-home` enforces **privileged** PSA because AdGuard runs `hostNetwork:true` to bind host `:53`/`:3000` (DNS + UI over NetBird). Container itself is hardened (drop ALL, only NET_BIND_SERVICE, read-only root fs). | Shares the host network namespace on Host 1; `:53` is firewall-restricted to the NetBird overlay interface `nb-wt0` (AR-05), not the public internet. | 2026-10-01 |
